...
def generate_medusa_buffers(medusa_choices, device="cuda"):
    ...
    # 按长度、字典序的优先顺序进行排序，得到一个二维列表，每个内层列表代表某一层的一条路径
    sorted_medusa_choices = sorted(medusa_choices, key=lambda x: (len(x), x))   
    medusa_len = len(sorted_medusa_choices) + 1 # medusa_len表示总路径数 + 1

    # Initialize depth_counts to keep track of how many choices have a particular depth
    depth_counts = []
    prev_depth = 0
    for path in sorted_medusa_choices:
        depth = len(path)   # 当前路径的长度，即当前层级，长度为 1 表示第一层，以此类推
        if depth != prev_depth:   # 如果当前层级与前一层级不同，则添加一个新元素到 depth_counts 列表中，表示发现了新层级
            depth_counts.append(0)
        depth_counts[depth - 1] += 1   # 将当前层级的计数器加1，所以depth_counts是用来记录每层有多少条路径的，
        prev_depth = depth
    
    # Create the attention mask for Medusa
    medusa_attn_mask = torch.eye(medusa_len, medusa_len)    # 生成单位对角矩阵，对角线为1，其余为0
    medusa_attn_mask[:, 0] = 1    # 第一列全为1，即所有 token 都可以看见第0个token——根节点
    # 按层遍历节点，构建注意力掩码矩阵。
    start = 0   # 每一层在 sorted_medusa_choices和 mask 中的起始位置
    for i in range(len(depth_counts)): # i从0到深度数-1，表示第i层（注意，这里i从0开始，但深度是从1开始）
        for j in range(depth_counts[i]):    # 遍历当前层的每条路径，j表示第i层的路径数，也是当前层的节点数
            cur_medusa_choice = sorted_medusa_choices[start + j]    # 获取当前节点的路径
            # retrieve ancestor position
            if len(cur_medusa_choice) == 1:     # 如果是第一层节点，已经可以关注根节点，所以跳过
                continue
            ancestor_idx = []
            for c in range(len(cur_medusa_choice) - 1):     # 遍历当前路径的每个前缀（除了完整路径自身）
                # 例：cur_medusa_choice = [0,1,2]，循环变量 c 取 0, 1 → 对应前缀 [0] 和 [0,1]。
                # 找到前缀路径在sorted_medusa_choices中的索引，然后+1（因为根节点占0索引）
                # [:c+1]取出 0 到 c 索引，长度为 c+1 的前缀。
                ancestor_idx.append(sorted_medusa_choices.index(cur_medusa_choice[:c+1]) + 1)
            # 设置当前节点可以关注这些祖先节点
            # ancestor_idx 是数组也没关系，对于下面的这句代码，假如ancestor_idx = [1,2]，则 medusa_attn_mask[j + start + 1, [1,2]] = 1，即第j + start + 1行，第1列和第2列都为1
            medusa_attn_mask[j + start + 1, ancestor_idx] = 1
        start += depth_counts[i]    # 更新start到下一层的开始

    # Generate tree indices for the Medusa structure
    medusa_tree_indices = torch.zeros(medusa_len, dtype=torch.long)
    medusa_tree_indices[0] = 0      # 根节点为0
    start = 0
    for i in range(len(depth_counts)):
        for j in range(depth_counts[i]):    # 同样遍历当前层的每条路径
            cur_medusa_choice = sorted_medusa_choices[start + j]    # 获取当前节点的路径
            # 计算当前节点的树索引：路径最后一个数字 + TOPK * i + 1，具体原理见笔记
    ...
    start = 0
    for i in range(len(depth_counts)):
        # 将当前层（第i层）的所有节点的位置ID设置为i+1，同一层的所有节点共享相同的位置ID
        medusa_position_ids[start + 1: start + depth_counts[i] + 1] = i + 1
        start += depth_counts[i]

    # Generate retrieval indices for Medusa structure verification
    # 为每个叶子节点记录从根节点到它的完整路径索引
    retrieve_indices_nest = []  # 存储嵌套的检索索引，内层是 retrieve_indice，用于检索当前路径的祖先节点
    retrieve_paths = []         # 存储已经处理过的路径，避免重复
    # 从最长的路径开始遍历（逆序）
    for i in range(len(sorted_medusa_choices)):
        cur_medusa_choice = sorted_medusa_choices[-i-1]     # 从最后一个开始
        retrieve_indice = []    # 当前路径的检索索引
        if cur_medusa_choice in retrieve_paths:
            continue
        else:
            #将所有前序路径在sorted_medusa_choices中的索引添加到retrieve_indice中
            for c in range(len(cur_medusa_choice)):
                retrieve_indice.append(sorted_medusa_choices.index(cur_medusa_choice[:c+1]))
                retrieve_paths.append(cur_medusa_choice[:c+1])
        retrieve_indices_nest.append(retrieve_indice)
    # 找到最长的检索序列长度，然后填充其他序列到相同长度，因为后面要做 tensor 化 + batch gather，必须等长。
    max_length = max([len(x) for x in retrieve_indices_nest]) # 所以从最长的路径开始遍历，这样max_length才能最早被确定
    retrieve_indices = [pad_path(path, max_length) for path in retrieve_indices_nest]
    # 转成 LongTensor 并统一 +1（把 0 号留给根节点）
    retrieve_indices = torch.tensor(retrieve_indices, dtype=torch.long)
    retrieve_indices = retrieve_indices + 1     # 因为0是根节点，所以retrieve_indices所有元素即索引加1，这样0就空出来，后面再在开头加0
    # 在每一行的开头都添加了一个0（根节点），retrieve_indices_nest形状变成[num_medusa_paths, max_length+1]
    # 这样 forward 时可以用 一行 gather 操作 就把 根 + 路径全部节点 的 logits 一次性抽出来
    retrieve_indices = torch.cat([torch.zeros((retrieve_indices.shape[0], 1), dtype=torch.long), retrieve_indices], dim=1)

    # Aggregate the generated buffers into a dictionary
    medusa_buffers = {
        "medusa_attn_mask": medusa_attn_mask.unsqueeze(0).unsqueeze(0),    # 将 [64,64] → [1,1,64,64] 以适应注意力机制
...
def generate_candidates(medusa_logits, logits, tree_indices, retrieve_indices, temperature = 0, posterior_threshold=0.3, posterior_alpha = 0.09, top_p=0.8, sampling = 'typical', fast = False):
    ...
    if temperature == 0 or fast:
        candidates_logit = torch.argmax(logits[:, -1]).unsqueeze(0) # 选择概率最高的token top-1
    else:
        if sampling == 'typical':    # 基于熵和阈值选择token
            candidates_logit = get_typical_one_token(logits[:, -1], temperature, posterior_threshold, posterior_alpha).squeeze(0)
        elif sampling == 'nucleus':  # 基于累积概率选择token
            candidates_logit = get_nucleus_one_token(logits[:, -1], temperature, top_p).squeeze(0)
        # typical 与 nucleus 都会返回 一个 token ID， .squeeze(0) 保证形状为 [] 或 [1] → 统一成标量
        else:
            raise NotImplementedError
    # Extract the TOPK candidates from the medusa logits.
    # TOPK在代码开头定义为了 10
    # medusa_logits ：[num_medusa_heads, batch_size, seq_len, vocab_size]
    # [:, 0, -1] 表示：所有头 → 第0个批次 → 最后一个位置的所有logits值，得到一个[num_medusa_heads, vocab_size]的tensor
    # 在[num_medusa_heads, vocab_size]的tensor的最后一个维度（词汇表维度）上选择前TOPK个最大值，得到一个[num_medusa_heads, TOPK]的tensor
    # .indices：只返回索引（token IDs），不返回对应的概率值
    candidates_medusa_logits = torch.topk(medusa_logits[:, 0, -1], TOPK, dim = -1).indices  # dim：沿着哪个维度进行选择

    # Combine the selected candidate from the original logits with the topk medusa logits.
    # 把主模型 1 个 + Medusa 10×heads 个拼在一起
    #.view(-1) 把 [num_heads, 10] 拉成 [num_heads*10]
    # 最终 candidates 形状 [1 + num_heads*10]，里面是所有可能出现在树节点上的 token ID，按照"根节点 + Head0的TOP10 + Head1的TOP10 + ..."的规律排列
    # 且Head i 的TOP10个token在 candidates 中是按概率从高到低排列
    candidates = torch.cat([candidates_logit, candidates_medusa_logits.view(-1)], dim=-1)   

    # Map the combined candidates to the tree indices to get tree candidates.
    # 按预先生成的 tree_indices 重排，得到 树节点顺序
    tree_candidates = candidates[tree_indices]

    # Extend the tree candidates by appending a zero.
    # 较短的路径会用pad token填充到与最长路径相同长度
...
def evaluate_posterior(
    ...
    if temperature == 0:
        ...
        ).int() # 得到一串01序列，表示每个位置是否预测正确
        candidates_accept_length = (torch.cumprod(posterior_mask, dim=1)).sum(dim=1) # cumprod是累积乘积，这样能得到最长正确序列的长度
